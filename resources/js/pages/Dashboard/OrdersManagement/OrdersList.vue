    ...
    <div class="col-md-12">
      <div class="card">
        <div class="card-header card-header-primary" style="text-align: center;">
          <h4 class="card-title">قائمة طلبات الزبائن</h4>
          <p class="card-category">الطلبات الموجودة في النظام</p>
        </div>
          ...
          <div class="table-responsive">
              <table class="table" style="width: 100%;">
                <thead class="text-primary">
                    ...
                    <th style="width: 20%;">اجمالي السعر</th>
                    <th>اجراءات</th>
                </thead>
                <tbody>
                  <tr v-for="(item, index) in Orders.data" :key="index">
                    ...
                    <td class="text-primary">
                      <router-link :to="{name:'admin.dashboard.order.details',params:{id:item.id}}" type="button" class="btn my_btn btn-sm">
...
<script>
  ...
  export default {
      ...
      computed:{
        ...mapState({
          Orders:state=>state.admin.Orders.orders,
          }),
      },

      created(){
        store.commit('admin/PleaseStartLoading');
        store.dispatch('admin/fetchOrders').then(()=>{
