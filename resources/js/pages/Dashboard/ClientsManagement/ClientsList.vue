    ...
    <div class="col-md-12">
      <div class="card">
        <div class="card-header card-header-primary" style="text-align: center;">
          ...
          <p class="card-category">العملاء الموجودين في النظام</p>
        </div>
        ...
        <div class="card-body" style="text-align:center;">
          <div class="table-responsive">
              <table class="table">
                ...
                <tbody>
                  <tr v-for="(item, index) in ClientsList.data" :key="index">
                    <td>{{index+1}}</td>
                    <td>{{item.name}}</td>
                    <td>{{item.phone}}</td>
                    <td class="text-primary">
                    <router-link :to="{name:'admin.dashboard.client.details',params:{id:item.id}}" type="button" class="btn my_btn btn-sm">
...
<script>
  ...
  export default {
      ...
      computed:{
        ...mapState({
          ClientsList:state=>state.admin.Clients.clients,
        }),
      },

      created(){
        store.commit('admin/PleaseStartLoading');
        store.dispatch('admin/fetchClients').then(()=>{
